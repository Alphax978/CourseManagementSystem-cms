// The following file contains code for viewing,deleting and adding course
...
public class CourseDetails extends JFrame implements ActionListener {
    ...
    JButton b1, b2;
    JTextField rr;
    String x[] = { "Course Name", "Semesters", "Course Code", "Duration", "Modules"}; // assigning column headers
    ...
    CourseDetails() {
        ...
        a1 = new JLabel("Enter Name  of the Course:");
        ...
        try {
            ...
            String sent = "select * from course_info";
            ResultSet res = ctv.x.executeQuery(sent);
            while (res.next()) {
                y[i][j++] = res.getString("course_name");
                y[i][j++] = res.getString("semesters");
                y[i][j++] = res.getString("course_code");
                y[i][j++] = res.getString("duration");
                y[i][j++] = res.getString("modules");
                i++;
                j = 0;
            }
            v1 = new JTable(y, x);
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == b1) {
            try {
                ...
                String delt = "delete from course_info where course_name = '" + gety + "'";
